...
<body class="min-h-screen pb-20">
        ...
        @if(count($cart) > 0)
            <div class="space-y-6">
                    ...
                    <div id="cart-item-{{ $id }}" class="glass p-6 rounded-3xl border border-white/5 flex items-center justify-between group transition duration-500">
                        <div class="flex items-center space-x-6">
                            <div class="w-20 h-20 rounded-2xl overflow-hidden border border-white/10 group-hover:border-gold/30 transition">
                                <img src="{{ str_starts_with($details['image'], 'http') ? $details['image'] : asset('storage/' . $details['image']) }}" class="w-full h-full object-cover" alt="{{ $details['name'] }}">
                            </div>
                            <div>
                ...
                <div class="mt-12 glass p-8 rounded-[2.5rem] border border-white/5 bg-white/[0.01]">
                    ...
                    <form action="{{ route('cart.checkout') }}" method="POST" id="checkout-form" enctype="multipart/form-data">
                        @csrf
                        ...
                        <div class="mb-8">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <label class="border border-white/10 bg-white/[0.03] rounded-2xl p-4 cursor-pointer hover:-translate-y-1 hover:shadow-lg hover:shadow-gold/10 hover:bg-white/[0.05] active:scale-95 transition-all duration-300 flex items-center relative group">
                                    <input type="radio" name="courier_type" value="standard" class="hidden peer" required checked onchange="calculateGrandTotal({{ $standardCost }})" />
                            ...
                            <input type="hidden" name="shipping_cost" id="shipping_cost" value="{{ $standardCost }}">
                        </div>
                        ...
                        <button type="submit" id="checkout-btn" class="w-full bg-gold text-[#000b1a] py-6 rounded-2xl font-black uppercase tracking-widest hover:bg-white hover:scale-[1.02] transition shadow-2xl shadow-gold/20 flex items-center justify-center">
            ...
            <div id="address-modal" class="fixed inset-0 z-[100] flex items-center justify-center hidden p-6">
                <!-- Backdrop -->
                <div class="absolute inset-0 bg-[#000b1a]/95 backdrop-blur-xl"></div>
                
                <!-- Modal Content -->
                <div class="relative glass max-w-lg w-full rounded-[2.5rem] border border-white/10 p-10 text-center shadow-2xl animate-in fade-in zoom-in duration-300">
                    ...
                    <h3 class="text-3xl font-black mb-4 tracking-tight">ALAMAT <span class="text-red-500">BELUM DIISI</span></h3>
                    <p class="text-gray-400 leading-relaxed mb-10">Maaf, Anda perlu melengkapi alamat pengiriman di profil Anda sebelum dapat melanjutkan proses pembayaran.</p>
                    
                    <div class="flex flex-col gap-4">
                        <a href="{{ route('profile.edit', ['redirect_to' => route('cart.index')]) }}" class="w-full bg-gold text-[#000b1a] py-5 rounded-2xl font-black uppercase tracking-widest hover:bg-white hover:scale-[1.02] transition flex items-center justify-center">
                            <i class="fas fa-user-edit mr-4"></i> Lengkapi Profil Sekarang
                        </a>
                        <button onclick="closeAddressModal()" class="w-full bg-white/5 text-gray-400 py-5 rounded-2xl font-bold uppercase tracking-widest hover:bg-white/10 transition">
                            Nanti Saja
                        </button>
                    </div>
                </div>
    ...
    <script>
        ...
        const hasAddress = @json(!empty(auth()->user()->address));
